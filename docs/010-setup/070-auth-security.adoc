...
**Source**: https://laravel.com/docs/12.x/fortify[Laravel Fortify Documentation]
...
|Install Fortify
|`php artisan fortify:install`
|Configuration and migrations published
|`config/fortify.php` exists

|3
|Run migrations
|`php artisan migrate`
|Two-factor authentication table created (if enabled)
|Tables exist in database

|4
|Configure features
|Edit `config/fortify.php` → `features` array
|Features enabled/disabled
|Config file updated

|5
|Verify service provider
|Check `bootstrap/providers.php` for FortifyServiceProvider
|Provider registered
|Provider listed

|6
|Create authentication views (if using Blade)
|Create login/register views that submit to Fortify routes
|Authentication forms created
|Forms accessible in browser
|===

=== 2.7. Feature Configuration
...
'features' => [
    ...
    Features::twoFactorAuthentication([
        'confirmPassword' => true,  // Require password confirmation before enabling 2FA
    ]),
],
----
...
----
'features' => [
    // Features::registration(),  // Disabled - no user registration
    Features::resetPasswords(),
    Features::emailVerification(),
    // ... other features
],
...
**Source**: https://docs.workos.com[WorkOS Documentation] | https://github.com/laravel/workos[Laravel WorkOS Package]
...
# Expected output shows version ^0.5
----
...
|Account created
|Can access WorkOS dashboard

|3
|Get WorkOS API key
|Copy API key from WorkOS dashboard
|API key obtained
|Key available to copy

|4
|Get WorkOS Client ID
|Copy Client ID from WorkOS dashboard
|Client ID obtained
|ID available

|5
|Configure environment variables
|Set `WORKOS_API_KEY` and `WORKOS_CLIENT_ID` in `.env`
...
|`php artisan vendor:publish --tag=workos-config`
|Config file created
|`config/workos.php` exists (if published)

|7
|Configure WorkOS in application
|Use WorkOS in your authentication flow
|WorkOS integrated
|SSO/login working
|===
...
|WORKOS_ENVIRONMENT
|WorkOS environment (production/staging)
|production
|===
...
=== 6.1. Issue: Registration Not Working
...
=== 6.2. Issue: WorkOS Connection Failed
...
* xref:080-queue-monitoring.adoc[queue-monitoring.adoc] - Set up Horizon and Octane

== 8. Navigation

xref:060-admin-panel.adoc[← Admin Panel] | xref:080-queue-monitoring.adoc[Next: Queue & Monitoring →]
